Shader "Ciconia Studio/Shader Painter/MetalRough/2Sided/Transparent/CutOut"
{
	Properties
	{
	...
	SubShader
	{
		...
		void surf( Input i , inout SurfaceOutputStandard o )
		{
			...
			float3 lerpResult5_g337 = lerp( float3(0,0,1) , UnpackNormal( tex2D( _BumpMap, ( uv_BumpMap * _TilingLayer1 ) ) ) , _NormalIntensity1);
			float3 Normalmap338 = lerpResult5_g337;
			...
			float4 temp_output_480_0 = ( _CiconiaStudiocutout * ( _Color * float4( desaturateVar16_g302 , 0.0 ) ) * _2Sided );
			...
			float fresnelNdotV4_g334 = dot( ase_worldNormal, ase_worldViewDir );
			float fresnelNode4_g334 = ( -0.05 + 1.0 * pow( 1.0 - fresnelNdotV4_g334, 6.0 ) );
			float4 Emissivemap375 = ( ( _EmissionColor * tex2D( _EmissionMap, ( uv_EmissionMap * _TilingLayer1 ) ) * _EmissiveIntensity1 ) + ( ( ( 0.95 * fresnelNode4_g334 ) + 0.05 ) * _FresnelStrength ) );
			o.Emission = Emissivemap375.rgb;
			float2 uv_MetallicGlossMap = i.uv_texcoord * _MetallicGlossMap_ST.xy + _MetallicGlossMap_ST.zw;
			float4 tex2DNode3_g332 = tex2D( _MetallicGlossMap, ( uv_MetallicGlossMap * _TilingLayer1 ) );
			float fresnelNdotV4_g336 = dot( ase_worldNormal, ase_worldViewDir );
			float fresnelNode4_g336 = ( -0.05 + 1.0 * pow( 1.0 - fresnelNdotV4_g336, 6.0 ) );
			float4 Metallic274 = ( ( _SpecularColor1 * tex2DNode3_g332 * _Metallic ) + ( ( ( 0.95 * fresnelNode4_g336 ) + 0.05 ) * _FresnelStrength ) );
			o.Metallic = Metallic274.r;
			float Roughness275 = ( tex2DNode3_g332.a * _Glossiness );
			o.Smoothness = Roughness275;
			float2 uv_OcclusionMap = i.uv_texcoord * _OcclusionMap_ST.xy + _OcclusionMap_ST.zw;
			float blendOpSrc2_g338 = tex2D( _OcclusionMap, ( uv_OcclusionMap * _TilingLayer1 ) ).r;
			float blendOpDest2_g338 = ( 1.0 - _AoIntensity1 );
			float Aomap351 = ( saturate( ( 1.0 - ( 1.0 - blendOpSrc2_g338 ) * ( 1.0 - blendOpDest2_g338 ) ) ));
