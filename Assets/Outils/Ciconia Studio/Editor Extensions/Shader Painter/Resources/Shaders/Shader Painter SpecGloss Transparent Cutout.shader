Shader "Ciconia Studio/Shader Painter/SpecGloss/Transparent/CutOut"
{
	Properties
	{
	...
	SubShader
	{
		...
		void surf( Input i , inout SurfaceOutputStandardSpecular o )
		{
			...
			float3 lerpResult5_g310 = lerp( float3(0,0,1) , UnpackNormal( tex2D( _BumpMap, ( uv_BumpMap * _TilingLayer1 ) ) ) , _NormalIntensity1);
			float3 Normalmap338 = lerpResult5_g310;
			...
			float4 temp_output_473_0 = ( _CiconiaStudiocutout * ( _Color * float4( desaturateVar16_g234 , 0.0 ) ) );
			...
			float fresnelNdotV4_g308 = dot( ase_worldNormal, ase_worldViewDir );
			float fresnelNode4_g308 = ( -0.05 + 1.0 * pow( 1.0 - fresnelNdotV4_g308, 6.0 ) );
			float4 Emissivemap375 = ( ( _EmissionColor * tex2D( _EmissionMap, ( uv_EmissionMap * _TilingLayer1 ) ) * _EmissiveIntensity1 ) + ( ( ( 0.95 * fresnelNode4_g308 ) + 0.05 ) * _FresnelStrength ) );
			o.Emission = Emissivemap375.rgb;
			float2 uv_SpecGlossMap = i.uv_texcoord * _SpecGlossMap_ST.xy + _SpecGlossMap_ST.zw;
			float4 tex2DNode3_g309 = tex2D( _SpecGlossMap, ( uv_SpecGlossMap * _TilingLayer1 ) );
			float fresnelNdotV4_g306 = dot( ase_worldNormal, ase_worldViewDir );
			float fresnelNode4_g306 = ( -0.05 + 1.0 * pow( 1.0 - fresnelNdotV4_g306, 6.0 ) );
			float4 Specularmap274 = ( ( _SpecularColor1 * tex2DNode3_g309 * _SpecularIntensity1 ) + ( ( ( 0.95 * fresnelNode4_g306 ) + 0.05 ) * _FresnelStrength ) );
			o.Specular = Specularmap274.rgb;
			float Glossiness275 = ( tex2DNode3_g309.a * _Glossiness1 );
			o.Smoothness = Glossiness275;
			float2 uv_OcclusionMap = i.uv_texcoord * _OcclusionMap_ST.xy + _OcclusionMap_ST.zw;
			float blendOpSrc2_g311 = tex2D( _OcclusionMap, ( uv_OcclusionMap * _TilingLayer1 ) ).r;
			float blendOpDest2_g311 = ( 1.0 - _AoIntensity1 );
			float Aomap351 = ( saturate( ( 1.0 - ( 1.0 - blendOpSrc2_g311 ) * ( 1.0 - blendOpDest2_g311 ) ) ));
