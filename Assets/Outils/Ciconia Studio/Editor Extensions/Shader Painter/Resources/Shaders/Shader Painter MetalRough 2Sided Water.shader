Shader "Ciconia Studio/Shader Painter/MetalRough/2Sided/Water"
{
	...
	SubShader
	{
		...
		void surf( Input i , inout SurfaceOutputStandard o )
		{
			...
			float3 lerpResult5_g438 = lerp( float3(0,0,1) , UnpackNormal( tex2D( _BumpMap, ( uv_BumpMap * _TilingLayer1 ) ) ) , _NormalIntensity1);
			float3 temp_output_443_0 = lerpResult5_g438;
			float3 _VectorBleu = float3(0,0,1);
			float2 uv_Normalmap2 = i.uv_texcoord * _Normalmap2_ST.xy + _Normalmap2_ST.zw;
			float cos12_g439 = cos( _Angle1 );
			float sin12_g439 = sin( _Angle1 );
			float2 rotator12_g439 = mul( ( uv_Normalmap2 * _TilingLayer2 ) - float2( 0.5,0.5 ) , float2x2( cos12_g439 , -sin12_g439 , sin12_g439 , cos12_g439 )) + float2( 0.5,0.5 );
			float2 temp_output_18_0_g439 = ( i.uv_texcoord / 4.0 );
			float2 panner16_g439 = ( ( _Time.y * _AnimationSpeed1 ) * float2( 0,0.6 ) + temp_output_18_0_g439);
			float3 tex2DNode4_g439 = UnpackNormal( tex2D( _Normalmap2, ( rotator12_g439 + panner16_g439 ) ) );
			float3 lerpResult5_g439 = lerp( _VectorBleu , tex2DNode4_g439 , _NormalIntensity2);
			float cos35_g439 = cos( _Angle2 );
			float sin35_g439 = sin( _Angle2 );
			float2 rotator35_g439 = mul( ( uv_Normalmap2 * _TilingLayer2 ) - float2( 0.5,0.5 ) , float2x2( cos35_g439 , -sin35_g439 , sin35_g439 , cos35_g439 )) + float2( 0.5,0.5 );
			float2 panner33_g439 = ( ( _Time.y * _AnimationSpeed2 ) * float2( 0,0.6 ) + temp_output_18_0_g439);
			float3 tex2DNode21_g439 = UnpackNormal( tex2D( _Normalmap2, ( rotator35_g439 + ( 1.0 - panner33_g439 ) ) ) );
			float3 lerpResult40_g439 = lerp( _VectorBleu , tex2DNode21_g439 , _NormalIntensity2);
			float3 temp_output_558_0 = BlendNormals( lerpResult5_g439 , lerpResult40_g439 );
			float2 uv_MainTex = i.uv_texcoord * _MainTex_ST.xy + _MainTex_ST.zw;
			float4 tex2DNode7_g433 = tex2D( _MainTex, ( uv_MainTex * _TilingLayer1 ) );
			float ADiffuse1210 = tex2DNode7_g433.a;
			float clampResult11_g436 = clamp( ( _Spread + (( 1.0 + _Contrast ) + (ADiffuse1210 - 0.0) * (-_Contrast - ( 1.0 + _Contrast )) / (1.0 - 0.0)) ) , 0.0 , 1.0 );
			...
			float temp_output_21_0_g436 = MaskBrush232;
			float blendOpSrc18_g436 = clampResult11_g436;
			float blendOpDest18_g436 = saturate( ( (-1.0 + (3.0 - 0.0) * (1.0 - -1.0) / (1.0 - 0.0)) + (( 1.0 + -6.0 ) + (temp_output_21_0_g436 - 0.0) * (6.0 - ( 1.0 + -6.0 )) / (1.0 - 0.0)) ) );
			float blendOpSrc31_g436 = lerp(clampResult11_g436,( saturate( ( blendOpSrc18_g436 * blendOpDest18_g436 ) )),_Heightmapcontstraintbymask);
			float blendOpDest31_g436 = temp_output_21_0_g436;
			float Mask246 = ( ( saturate( ( blendOpSrc31_g436 + blendOpDest31_g436 ) )) * _MaskOpacity );
			...
			float4 lerpResult251 = lerp( ( _Color * float4( desaturateVar16_g433 , 0.0 ) ) , ( float4( desaturateVar16_g433 , 0.0 ) * _Color2 ) , Mask246);
			...
			float4 Emissivemap375 = ( ( _EmissionColor * tex2D( _EmissionMap, ( uv_EmissionMap * _TilingLayer1 ) ) * _EmissiveIntensity1 ) + ( ( ( ( 0.95 * fresnelNode4_g440 ) + 0.05 ) * _FresnelStrength ) + ( texCUBENode507 * ( texCUBENode507.a * ( _ReflectionIntensity / 10.0 ) * Mask246 ) ) ) );
			o.Emission = Emissivemap375.rgb;
			float2 uv_MetallicGlossMap = i.uv_texcoord * _MetallicGlossMap_ST.xy + _MetallicGlossMap_ST.zw;
			float4 tex2DNode3_g445 = tex2D( _MetallicGlossMap, ( uv_MetallicGlossMap * _TilingLayer1 ) );
			float temp_output_17_0_g445 = Mask246;
			float4 lerpResult16_g445 = lerp( ( _SpecularColor1 * tex2DNode3_g445 * _Metallic ) , ( _SpecularColor2 * tex2DNode3_g445 * _Metallic2 ) , temp_output_17_0_g445);
			float fresnelNdotV4_g447 = dot( ase_worldNormal, ase_worldViewDir );
			float fresnelNode4_g447 = ( -0.05 + 1.0 * pow( 1.0 - fresnelNdotV4_g447, 6.0 ) );
			float4 Metallic274 = ( lerpResult16_g445 + ( ( ( 0.95 * fresnelNode4_g447 ) + 0.05 ) * _FresnelStrength ) );
			o.Metallic = Metallic274.r;
			float lerpResult22_g445 = lerp( ( tex2DNode3_g445.a * _Glossiness ) , _Roughness2 , temp_output_17_0_g445);
			float Roughness275 = lerpResult22_g445;
			o.Smoothness = Roughness275;
			float2 uv_OcclusionMap = i.uv_texcoord * _OcclusionMap_ST.xy + _OcclusionMap_ST.zw;
			float blendOpSrc2_g446 = tex2D( _OcclusionMap, ( uv_OcclusionMap * _TilingLayer1 ) ).r;
			float blendOpDest2_g446 = ( 1.0 - _AoIntensity1 );
			float blendOpSrc42_g439 = tex2DNode4_g439.b;
			float blendOpDest42_g439 = tex2DNode21_g439.b;
			float SpecularWave549 = ( saturate( (( blendOpDest42_g439 > 0.5 ) ? ( 1.0 - ( 1.0 - 2.0 * ( blendOpDest42_g439 - 0.5 ) ) * ( 1.0 - blendOpSrc42_g439 ) ) : ( 2.0 * blendOpDest42_g439 * blendOpSrc42_g439 ) ) ));
			float blendOpSrc552 = SpecularWave549;
			float blendOpDest552 = 0.0;
			float lerpResult362 = lerp( ( saturate( ( 1.0 - ( 1.0 - blendOpSrc2_g446 ) * ( 1.0 - blendOpDest2_g446 ) ) )) , ( saturate( ( 1.0 - ( ( 1.0 - blendOpDest552) / blendOpSrc552) ) )) , Mask246);
