Shader "Ciconia Studio/Shader Painter/SpecGloss/Water"
{
	Properties
	{
	...
	SubShader
	{
		...
		void surf( Input i , inout SurfaceOutputStandardSpecular o )
		{
			...
			float3 lerpResult5_g432 = lerp( float3(0,0,1) , UnpackNormal( tex2D( _BumpMap, ( uv_BumpMap * _TilingLayer1 ) ) ) , _NormalIntensity1);
			float3 temp_output_443_0 = lerpResult5_g432;
			float3 _VectorBleu = float3(0,0,1);
			float2 uv_Normalmap2 = i.uv_texcoord * _Normalmap2_ST.xy + _Normalmap2_ST.zw;
			float cos12_g431 = cos( _Angle1 );
			float sin12_g431 = sin( _Angle1 );
			float2 rotator12_g431 = mul( ( uv_Normalmap2 * _TilingLayer2 ) - float2( 0.5,0.5 ) , float2x2( cos12_g431 , -sin12_g431 , sin12_g431 , cos12_g431 )) + float2( 0.5,0.5 );
			float2 temp_output_18_0_g431 = ( i.uv_texcoord / 4.0 );
			float2 panner16_g431 = ( ( _Time.y * _AnimationSpeed1 ) * float2( 0,0.6 ) + temp_output_18_0_g431);
			float3 tex2DNode4_g431 = UnpackNormal( tex2D( _Normalmap2, ( rotator12_g431 + panner16_g431 ) ) );
			float3 lerpResult5_g431 = lerp( _VectorBleu , tex2DNode4_g431 , _NormalIntensity2);
			float cos35_g431 = cos( _Angle2 );
			float sin35_g431 = sin( _Angle2 );
			float2 rotator35_g431 = mul( ( uv_Normalmap2 * _TilingLayer2 ) - float2( 0.5,0.5 ) , float2x2( cos35_g431 , -sin35_g431 , sin35_g431 , cos35_g431 )) + float2( 0.5,0.5 );
			float2 panner33_g431 = ( ( _Time.y * _AnimationSpeed2 ) * float2( 0,0.6 ) + temp_output_18_0_g431);
			float3 tex2DNode21_g431 = UnpackNormal( tex2D( _Normalmap2, ( rotator35_g431 + ( 1.0 - panner33_g431 ) ) ) );
			float3 lerpResult40_g431 = lerp( _VectorBleu , tex2DNode21_g431 , _NormalIntensity2);
			float3 temp_output_558_0 = BlendNormals( lerpResult5_g431 , lerpResult40_g431 );
			float2 uv_MainTex = i.uv_texcoord * _MainTex_ST.xy + _MainTex_ST.zw;
			float4 tex2DNode7_g425 = tex2D( _MainTex, ( uv_MainTex * _TilingLayer1 ) );
			float ADiffuse1210 = tex2DNode7_g425.a;
			float clampResult11_g430 = clamp( ( _Spread + (( 1.0 + _Contrast ) + (ADiffuse1210 - 0.0) * (-_Contrast - ( 1.0 + _Contrast )) / (1.0 - 0.0)) ) , 0.0 , 1.0 );
			float2 uv_Brush = i.uv_texcoord * _Brush_ST.xy + _Brush_ST.zw;
			float2 appendResult19_g426 = (float2(_Brushposition.x , _Brushposition.y));
			float cos23_g426 = cos( _Rotate );
			float sin23_g426 = sin( _Rotate );
			float2 rotator23_g426 = mul( ( uv_Brush + ( appendResult19_g426 * _Pensize ) ) - float2( 0.5,0.5 ) , float2x2( cos23_g426 , -sin23_g426 , sin23_g426 , cos23_g426 )) + float2( 0.5,0.5 );
			float temp_output_25_0_g426 = ( tex2D( _Brush, rotator23_g426 ).a * _Opacity );
			float2 uv_Mask = i.uv_texcoord * _Mask_ST.xy + _Mask_ST.zw;
			float temp_output_33_0_g426 = ( tex2D( _Mask, uv_Mask ).r * _OpacityDepth );
			float temp_output_31_0_g426 = min( ( 1.0 - temp_output_25_0_g426 ) , temp_output_33_0_g426 );
			float ifLocalVar38_g426 = 0;
			if( lerp(0.0,1.0,_Erase) == 0.0 )
				ifLocalVar38_g426 = max( temp_output_25_0_g426 , temp_output_33_0_g426 );
			else
				ifLocalVar38_g426 = temp_output_31_0_g426;
			float MaskBrush232 = ifLocalVar38_g426;
			float temp_output_21_0_g430 = MaskBrush232;
			float blendOpSrc18_g430 = clampResult11_g430;
			float blendOpDest18_g430 = saturate( ( (-1.0 + (3.0 - 0.0) * (1.0 - -1.0) / (1.0 - 0.0)) + (( 1.0 + -6.0 ) + (temp_output_21_0_g430 - 0.0) * (6.0 - ( 1.0 + -6.0 )) / (1.0 - 0.0)) ) );
			float blendOpSrc31_g430 = lerp(clampResult11_g430,( saturate( ( blendOpSrc18_g430 * blendOpDest18_g430 ) )),_Heightmapcontstraintbymask);
			float blendOpDest31_g430 = temp_output_21_0_g430;
			float Mask246 = ( ( saturate( ( blendOpSrc31_g430 + blendOpDest31_g430 ) )) * _MaskOpacity );
			...
			float clampResult15_g425 = clamp( _Desaturation1 , 0.0 , 1.0 );
			float3 desaturateInitialColor16_g425 = (temp_cast_3 + (pow( tex2DNode7_g425 , temp_cast_0 ) - temp_cast_1) * (temp_cast_4 - temp_cast_3) / (temp_cast_2 - temp_cast_1)).rgb;
			float desaturateDot16_g425 = dot( desaturateInitialColor16_g425, float3( 0.299, 0.587, 0.114 ));
			float3 desaturateVar16_g425 = lerp( desaturateInitialColor16_g425, desaturateDot16_g425.xxx, clampResult15_g425 );
			float4 lerpResult251 = lerp( ( _Color * float4( desaturateVar16_g425 , 0.0 ) ) , ( float4( desaturateVar16_g425 , 0.0 ) * _Color2 ) , Mask246);
			...
			float fresnelNdotV4_g433 = dot( ase_worldNormal, ase_worldViewDir );
			float fresnelNode4_g433 = ( -0.05 + 1.0 * pow( 1.0 - fresnelNdotV4_g433, 6.0 ) );
			float4 texCUBENode507 = texCUBElod( _Cubemap, float4( WorldReflectionVector( i , Normalmap338 ), _BlurReflection) );
			float4 Emissivemap375 = ( ( _EmissionColor * tex2D( _EmissionMap, ( uv_EmissionMap * _TilingLayer1 ) ) * _EmissiveIntensity1 ) + ( ( ( ( 0.95 * fresnelNode4_g433 ) + 0.05 ) * _FresnelStrength ) + ( texCUBENode507 * ( texCUBENode507.a * ( _ReflectionIntensity / 10.0 ) * Mask246 ) ) ) );
			o.Emission = Emissivemap375.rgb;
			float2 uv_SpecGlossMap = i.uv_texcoord * _SpecGlossMap_ST.xy + _SpecGlossMap_ST.zw;
			float4 tex2DNode3_g438 = tex2D( _SpecGlossMap, ( uv_SpecGlossMap * _TilingLayer1 ) );
			float temp_output_17_0_g438 = Mask246;
			float4 lerpResult16_g438 = lerp( ( _SpecularColor1 * tex2DNode3_g438 * _SpecularIntensity1 ) , ( _SpecularColor2 * tex2DNode3_g438 * _SpecularIntensity2 ) , temp_output_17_0_g438);
			float fresnelNdotV4_g439 = dot( ase_worldNormal, ase_worldViewDir );
			float fresnelNode4_g439 = ( -0.05 + 1.0 * pow( 1.0 - fresnelNdotV4_g439, 6.0 ) );
			float4 Specularmap274 = ( lerpResult16_g438 + ( ( ( 0.95 * fresnelNode4_g439 ) + 0.05 ) * _FresnelStrength ) );
			o.Specular = Specularmap274.rgb;
			float lerpResult22_g438 = lerp( ( tex2DNode3_g438.a * _Glossiness1 ) , _Glossiness2 , temp_output_17_0_g438);
			float Glossiness275 = lerpResult22_g438;
			o.Smoothness = Glossiness275;
			float2 uv_OcclusionMap = i.uv_texcoord * _OcclusionMap_ST.xy + _OcclusionMap_ST.zw;
			float blendOpSrc2_g441 = tex2D( _OcclusionMap, ( uv_OcclusionMap * _TilingLayer1 ) ).r;
			float blendOpDest2_g441 = ( 1.0 - _AoIntensity1 );
			float blendOpSrc42_g431 = tex2DNode4_g431.b;
			float blendOpDest42_g431 = tex2DNode21_g431.b;
			float SpecularWave549 = ( saturate( (( blendOpDest42_g431 > 0.5 ) ? ( 1.0 - ( 1.0 - 2.0 * ( blendOpDest42_g431 - 0.5 ) ) * ( 1.0 - blendOpSrc42_g431 ) ) : ( 2.0 * blendOpDest42_g431 * blendOpSrc42_g431 ) ) ));
			float blendOpSrc552 = SpecularWave549;
			float blendOpDest552 = 0.0;
			float lerpResult362 = lerp( ( saturate( ( 1.0 - ( 1.0 - blendOpSrc2_g441 ) * ( 1.0 - blendOpDest2_g441 ) ) )) , ( saturate( ( 1.0 - ( ( 1.0 - blendOpDest552) / blendOpSrc552) ) )) , Mask246);
