Shader "Ciconia Studio/Shader Painter/MetalRough/2Sided/Standard"
{
	Properties
	{
	...
	SubShader
	{
		...
		void surf( Input i , inout SurfaceOutputStandard o )
		{
			...
			float3 lerpResult5_g383 = lerp( float3(0,0,1) , UnpackNormal( tex2D( _BumpMap, ( uv_BumpMap * _TilingLayer1 ) ) ) , _NormalIntensity1);
			float3 temp_output_478_0 = lerpResult5_g383;
			float2 uv_Normalmap2 = i.uv_texcoord * _Normalmap2_ST.xy + _Normalmap2_ST.zw;
			float3 lerpResult5_g384 = lerp( float3(0,0,1) , UnpackNormal( tex2D( _Normalmap2, ( uv_Normalmap2 * _TilingLayer2 ) ) ) , _NormalIntensity2);
			float3 temp_output_479_0 = lerpResult5_g384;
			float2 uv_MainTex = i.uv_texcoord * _MainTex_ST.xy + _MainTex_ST.zw;
			float4 tex2DNode7_g373 = tex2D( _MainTex, ( uv_MainTex * _TilingLayer1 ) );
			float ADiffuse1210 = tex2DNode7_g373.a;
			float clampResult11_g379 = clamp( ( _Spread + (( 1.0 + _Contrast ) + (ADiffuse1210 - 0.0) * (-_Contrast - ( 1.0 + _Contrast )) / (1.0 - 0.0)) ) , 0.0 , 1.0 );
			float2 uv_Brush = i.uv_texcoord * _Brush_ST.xy + _Brush_ST.zw;
			float2 appendResult19_g378 = (float2(_Brushposition.x , _Brushposition.y));
			float cos23_g378 = cos( _Rotate );
			float sin23_g378 = sin( _Rotate );
			float2 rotator23_g378 = mul( ( uv_Brush + ( appendResult19_g378 * _Pensize ) ) - float2( 0.5,0.5 ) , float2x2( cos23_g378 , -sin23_g378 , sin23_g378 , cos23_g378 )) + float2( 0.5,0.5 );
			float temp_output_25_0_g378 = ( tex2D( _Brush, rotator23_g378 ).a * _Opacity );
			float2 uv_Mask = i.uv_texcoord * _Mask_ST.xy + _Mask_ST.zw;
			float temp_output_33_0_g378 = ( tex2D( _Mask, uv_Mask ).r * _OpacityDepth );
			float temp_output_31_0_g378 = min( ( 1.0 - temp_output_25_0_g378 ) , temp_output_33_0_g378 );
			float ifLocalVar38_g378 = 0;
			if( lerp(0.0,1.0,_Erase) == 0.0 )
				ifLocalVar38_g378 = max( temp_output_25_0_g378 , temp_output_33_0_g378 );
			else
				ifLocalVar38_g378 = temp_output_31_0_g378;
			float MaskBrush232 = ifLocalVar38_g378;
			float temp_output_21_0_g379 = MaskBrush232;
			float blendOpSrc18_g379 = clampResult11_g379;
			float blendOpDest18_g379 = saturate( ( (-1.0 + (3.0 - 0.0) * (1.0 - -1.0) / (1.0 - 0.0)) + (( 1.0 + -6.0 ) + (temp_output_21_0_g379 - 0.0) * (6.0 - ( 1.0 + -6.0 )) / (1.0 - 0.0)) ) );
			float blendOpSrc31_g379 = lerp(clampResult11_g379,( saturate( ( blendOpSrc18_g379 * blendOpDest18_g379 ) )),_Heightmapcontstraintbymask);
			float blendOpDest31_g379 = temp_output_21_0_g379;
			float Mask246 = ( ( saturate( ( blendOpSrc31_g379 + blendOpDest31_g379 ) )) * _MaskOpacity );
			float3 lerpResult347 = lerp( temp_output_478_0 , temp_output_479_0 , Mask246);
			float3 lerpResult420 = lerp( temp_output_478_0 , BlendNormals( temp_output_478_0 , temp_output_479_0 ) , Mask246);
			...
			float clampResult15_g373 = clamp( _Desaturation1 , 0.0 , 1.0 );
			float3 desaturateInitialColor16_g373 = (temp_cast_3 + (pow( tex2DNode7_g373 , temp_cast_0 ) - temp_cast_1) * (temp_cast_4 - temp_cast_3) / (temp_cast_2 - temp_cast_1)).rgb;
			float desaturateDot16_g373 = dot( desaturateInitialColor16_g373, float3( 0.299, 0.587, 0.114 ));
			float3 desaturateVar16_g373 = lerp( desaturateInitialColor16_g373, desaturateDot16_g373.xxx, clampResult15_g373 );
			float2 uv_Diffusemap2 = i.uv_texcoord * _Diffusemap2_ST.xy + _Diffusemap2_ST.zw;
			float4 tex2DNode7_g380 = tex2D( _Diffusemap2, ( uv_Diffusemap2 * _TilingLayer2 ) );
			...
			float clampResult15_g380 = clamp( _Desaturation2 , 0.0 , 1.0 );
			float3 desaturateInitialColor16_g380 = (temp_cast_10 + (pow( tex2DNode7_g380 , temp_cast_7 ) - temp_cast_8) * (temp_cast_11 - temp_cast_10) / (temp_cast_9 - temp_cast_8)).rgb;
			float desaturateDot16_g380 = dot( desaturateInitialColor16_g380, float3( 0.299, 0.587, 0.114 ));
			float3 desaturateVar16_g380 = lerp( desaturateInitialColor16_g380, desaturateDot16_g380.xxx, clampResult15_g380 );
			float4 lerpResult251 = lerp( ( _Color * float4( desaturateVar16_g373 , 0.0 ) ) , ( _Color2 * float4( desaturateVar16_g380 , 0.0 ) ) , Mask246);
			float4 temp_output_467_0 = ( _CiconiaStudioBlend2 * lerpResult251 * _2Sided );
			...
			float fresnelNdotV4_g394 = dot( ase_worldNormal, ase_worldViewDir );
			float fresnelNode4_g394 = ( -0.05 + 1.0 * pow( 1.0 - fresnelNdotV4_g394, 6.0 ) );
			float4 Emissivemap375 = ( lerpResult378 + ( ( ( 0.95 * fresnelNode4_g394 ) + 0.05 ) * _FresnelStrength ) );
			o.Emission = Emissivemap375.rgb;
			float2 uv_MetallicGlossMap = i.uv_texcoord * _MetallicGlossMap_ST.xy + _MetallicGlossMap_ST.zw;
			float4 tex2DNode3_g390 = tex2D( _MetallicGlossMap, ( uv_MetallicGlossMap * _TilingLayer1 ) );
			float2 uv_MetallicRoughA2 = i.uv_texcoord * _MetallicRoughA2_ST.xy + _MetallicRoughA2_ST.zw;
			float4 tex2DNode3_g388 = tex2D( _MetallicRoughA2, ( uv_MetallicRoughA2 * _TilingLayer2 ) );
			float4 lerpResult266 = lerp( ( _SpecularColor1 * tex2DNode3_g390 * _Metallic ) , ( _SpecularColor2 * tex2DNode3_g388 * _Metallic2 ) , Mask246);
			float fresnelNdotV4_g395 = dot( ase_worldNormal, ase_worldViewDir );
			float fresnelNode4_g395 = ( -0.05 + 1.0 * pow( 1.0 - fresnelNdotV4_g395, 6.0 ) );
			float4 Metallic274 = ( lerpResult266 + ( ( ( 0.95 * fresnelNode4_g395 ) + 0.05 ) * _FresnelStrength ) );
			o.Metallic = Metallic274.r;
			float lerpResult336 = lerp( ( tex2DNode3_g390.a * _Glossiness ) , ( tex2DNode3_g388.a * _Roughness2 ) , Mask246);
			float Roughness275 = lerpResult336;
			o.Smoothness = Roughness275;
			float2 uv_OcclusionMap = i.uv_texcoord * _OcclusionMap_ST.xy + _OcclusionMap_ST.zw;
			float blendOpSrc2_g396 = tex2D( _OcclusionMap, ( uv_OcclusionMap * _TilingLayer1 ) ).r;
			float blendOpDest2_g396 = ( 1.0 - _AoIntensity1 );
			float4 temp_cast_20 = (( saturate( ( 1.0 - ( 1.0 - blendOpSrc2_g396 ) * ( 1.0 - blendOpDest2_g396 ) ) ))).xxxx;
			...
			float4 blendOpSrc2_g393 = tex2D( _AmbientOcclusionmap2, ( uv_AmbientOcclusionmap2 * _TilingLayer2 ) );
			float4 blendOpDest2_g393 = temp_cast_21;
			float4 lerpResult362 = lerp( temp_cast_20 , ( saturate( ( 1.0 - ( 1.0 - blendOpSrc2_g393 ) * ( 1.0 - blendOpDest2_g393 ) ) )) , Mask246);
