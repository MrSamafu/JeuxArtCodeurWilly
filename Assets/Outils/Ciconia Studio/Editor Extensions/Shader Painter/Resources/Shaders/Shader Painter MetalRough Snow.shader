Shader "Ciconia Studio/Shader Painter/MetalRough/Snow"
{
	Properties
	{
	...
	SubShader
	{
		...
		void surf( Input i , inout SurfaceOutputStandard o )
		{
			...
			float3 temp_output_479_0 = lerpResult5_g438;
			float2 uv_Normalmap2 = i.uv_texcoord * _Normalmap2_ST.xy + _Normalmap2_ST.zw;
			float3 lerpResult5_g439 = lerp( float3(0,0,1) , UnpackNormal( tex2D( _Normalmap2, ( uv_Normalmap2 * _TilingLayer2 ) ) ) , _NormalIntensity2);
			float3 temp_output_448_0 = lerpResult5_g439;
			...
			float4 temp_output_467_0 = ( _CiconiaStudioSnow * lerpResult251 );
			...
			float3 worldToObj45_g436 = mul( unity_WorldToObject, float4( ase_worldPos, 1 ) ).xyz;
			float3 ase_worldViewDir = normalize( UnityWorldSpaceViewDir( ase_worldPos ) );
			float3 temp_output_9_0_g436 = ( ( ( worldToObj45_g436 / 4.0 ) - ase_worldViewDir ) * ( _TilingCrystals / 10.0 ) );
			float3 weightedBlendVar32_g436 = abs( ase_vertexNormal );
			float4 weightedBlend32_g436 = ( weightedBlendVar32_g436.x*tex2D( _Crystalsmap, (temp_output_9_0_g436).yz ) + weightedBlendVar32_g436.y*tex2D( _Crystalsmap, (temp_output_9_0_g436).xz ) + weightedBlendVar32_g436.z*tex2D( _Crystalsmap, (temp_output_9_0_g436).xy ) );
			float clampResult33_g436 = clamp( _Desaturatecrystals , 0.0 , 1.0 );
			float3 desaturateInitialColor36_g436 = weightedBlend32_g436.rgb;
			float desaturateDot36_g436 = dot( desaturateInitialColor36_g436, float3( 0.299, 0.587, 0.114 ));
			float3 desaturateVar36_g436 = lerp( desaturateInitialColor36_g436, desaturateDot36_g436.xxx, clampResult33_g436 );
			float lerpResult30_g436 = lerp( -3.5 , 1.0 , _MaskAmount);
			...
			float4 SnowEmission485 = ( ( float4( ( desaturateVar36_g436 * ( _CrystalsIntensity * 20.0 ) ) , 0.0 ) * saturate( ( lerpResult30_g436 + (temp_cast_22 + (tex2D( _DotMask, ( uv_DotMask * _TilingMask ) ) - temp_cast_20) * (temp_cast_23 - temp_cast_22) / (temp_cast_21 - temp_cast_20)) ) ) ) * AoLayer2490 );
			...
			float4 tex2DNode3_g441 = tex2D( _MetallicGlossMap, ( uv_MetallicGlossMap * _TilingLayer1 ) );
			...
			float4 lerpResult266 = lerp( ( _SpecularColor1 * tex2DNode3_g441 * _Metallic ) , ( _SpecularColor2 * tex2DNode3_g443 * _Metallic2 ) , Mask246);
			...
			float4 Metallic274 = ( lerpResult266 + ( ( ( 0.95 * fresnelNode4_g447 ) + 0.05 ) * _FresnelStrength ) );
			o.Metallic = Metallic274.r;
			float lerpResult336 = lerp( ( tex2DNode3_g441.a * _Glossiness ) , ( tex2DNode3_g443.a * _Roughness2 ) , Mask246);
			...
			float lerpResult362 = lerp( ( saturate( ( 1.0 - ( 1.0 - blendOpSrc2_g446 ) * ( 1.0 - blendOpDest2_g446 ) ) )) , temp_output_488_0 , Mask246);
