Shader "Ciconia Studio/Shader Painter/SpecGloss/2Sided/Standard"
{
	...
	SubShader
	{
		...
		void surf( Input i , inout SurfaceOutputStandardSpecular o )
		{
			...
			float4 tex2DNode7_g42 = tex2D( _MainTex, ( uv_MainTex * _TilingLayer1 ) );
			float ADiffuse1210 = tex2DNode7_g42.a;
			float clampResult11_g44 = clamp( ( _Spread + (( 1.0 + _Contrast ) + (ADiffuse1210 - 0.0) * (-_Contrast - ( 1.0 + _Contrast )) / (1.0 - 0.0)) ) , 0.0 , 1.0 );
			...
			float temp_output_21_0_g44 = MaskBrush232;
			float blendOpSrc18_g44 = clampResult11_g44;
			float blendOpDest18_g44 = saturate( ( (-1.0 + (3.0 - 0.0) * (1.0 - -1.0) / (1.0 - 0.0)) + (( 1.0 + -6.0 ) + (temp_output_21_0_g44 - 0.0) * (6.0 - ( 1.0 + -6.0 )) / (1.0 - 0.0)) ) );
			float blendOpSrc31_g44 = lerp(clampResult11_g44,( saturate( ( blendOpSrc18_g44 * blendOpDest18_g44 ) )),_Heightmapcontstraintbymask);
			float blendOpDest31_g44 = temp_output_21_0_g44;
			float Mask246 = ( ( saturate( ( blendOpSrc31_g44 + blendOpDest31_g44 ) )) * _MaskOpacity );
			...
			float clampResult15_g42 = clamp( _Desaturation1 , 0.0 , 1.0 );
			float3 desaturateInitialColor16_g42 = (temp_cast_3 + (pow( tex2DNode7_g42 , temp_cast_0 ) - temp_cast_1) * (temp_cast_4 - temp_cast_3) / (temp_cast_2 - temp_cast_1)).rgb;
			float desaturateDot16_g42 = dot( desaturateInitialColor16_g42, float3( 0.299, 0.587, 0.114 ));
			float3 desaturateVar16_g42 = lerp( desaturateInitialColor16_g42, desaturateDot16_g42.xxx, clampResult15_g42 );
			float2 uv_Diffusemap2 = i.uv_texcoord * _Diffusemap2_ST.xy + _Diffusemap2_ST.zw;
			float4 tex2DNode7_g45 = tex2D( _Diffusemap2, ( uv_Diffusemap2 * _TilingLayer2 ) );
			...
			float clampResult15_g45 = clamp( _Desaturation2 , 0.0 , 1.0 );
			float3 desaturateInitialColor16_g45 = (temp_cast_10 + (pow( tex2DNode7_g45 , temp_cast_7 ) - temp_cast_8) * (temp_cast_11 - temp_cast_10) / (temp_cast_9 - temp_cast_8)).rgb;
			float desaturateDot16_g45 = dot( desaturateInitialColor16_g45, float3( 0.299, 0.587, 0.114 ));
			float3 desaturateVar16_g45 = lerp( desaturateInitialColor16_g45, desaturateDot16_g45.xxx, clampResult15_g45 );
			float4 lerpResult251 = lerp( ( _Color * float4( desaturateVar16_g42 , 0.0 ) ) , ( _Color2 * float4( desaturateVar16_g45 , 0.0 ) ) , Mask246);
			float4 temp_output_482_0 = ( _CiconiaStudioBlend2 * lerpResult251 * _2Sided );
			...
			float4 Emissivemap375 = ( lerpResult378 + ( ( ( 0.95 * fresnelNode4_g69 ) + 0.05 ) * _FresnelStrength ) );
			o.Emission = Emissivemap375.rgb;
			float2 uv_SpecGlossMap = i.uv_texcoord * _SpecGlossMap_ST.xy + _SpecGlossMap_ST.zw;
			float4 tex2DNode3_g63 = tex2D( _SpecGlossMap, ( uv_SpecGlossMap * _TilingLayer1 ) );
			float2 uv_SpecularmapGlossA2 = i.uv_texcoord * _SpecularmapGlossA2_ST.xy + _SpecularmapGlossA2_ST.zw;
			float4 tex2DNode3_g62 = tex2D( _SpecularmapGlossA2, ( uv_SpecularmapGlossA2 * _TilingLayer2 ) );
			float4 lerpResult266 = lerp( ( _SpecularColor1 * tex2DNode3_g63 * _SpecularIntensity1 ) , ( _SpecularColor2 * tex2DNode3_g62 * _SpecularIntensity2 ) , Mask246);
			float fresnelNdotV4_g68 = dot( ase_worldNormal, ase_worldViewDir );
			float fresnelNode4_g68 = ( -0.05 + 1.0 * pow( 1.0 - fresnelNdotV4_g68, 6.0 ) );
			float4 Specularmap274 = ( lerpResult266 + ( ( ( 0.95 * fresnelNode4_g68 ) + 0.05 ) * _FresnelStrength ) );
			o.Specular = Specularmap274.rgb;
			float lerpResult336 = lerp( ( tex2DNode3_g63.a * _Glossiness1 ) , ( tex2DNode3_g62.a * _Glossiness2 ) , Mask246);
			float Glossiness275 = lerpResult336;
			o.Smoothness = Glossiness275;
			float2 uv_OcclusionMap = i.uv_texcoord * _OcclusionMap_ST.xy + _OcclusionMap_ST.zw;
			float blendOpSrc2_g71 = tex2D( _OcclusionMap, ( uv_OcclusionMap * _TilingLayer1 ) ).r;
			float blendOpDest2_g71 = ( 1.0 - _AoIntensity1 );
			float4 temp_cast_20 = (( saturate( ( 1.0 - ( 1.0 - blendOpSrc2_g71 ) * ( 1.0 - blendOpDest2_g71 ) ) ))).xxxx;
