Shader "Ciconia Studio/Shader Painter/MetalRough/Standard Multi Layers"
{
	Properties
	{
	...
	SubShader
	{
		...
		void surf( Input i , inout SurfaceOutputStandard o )
		{
			...
			float3 lerpResult5_g407 = lerp( float3(0,0,1) , UnpackNormal( tex2D( _BumpMap, ( uv_BumpMap * TilingLayer1478 ) ) ) , _NormalIntensity1);
			float3 temp_output_443_0 = lerpResult5_g407;
			float2 uv_Normalmap2 = i.uv_texcoord * _Normalmap2_ST.xy + _Normalmap2_ST.zw;
			float TilingLayer2479 = _TilingLayer2;
			float3 lerpResult5_g408 = lerp( float3(0,0,1) , UnpackNormal( tex2D( _Normalmap2, ( uv_Normalmap2 * TilingLayer2479 ) ) ) , _NormalIntensity2);
			...
			float3 lerpResult347 = lerp( temp_output_443_0 , lerpResult5_g408 , MaskR246);
			float2 uv_Normalmap3 = i.uv_texcoord * _Normalmap3_ST.xy + _Normalmap3_ST.zw;
			float TilingLayer3480 = _TilingLayer3;
			float3 lerpResult5_g420 = lerp( float3(0,0,1) , UnpackNormal( tex2D( _Normalmap3, ( uv_Normalmap3 * TilingLayer3480 ) ) ) , _NormalIntensity3);
			...
			float3 lerpResult524 = lerp( lerpResult347 , lerpResult5_g420 , MaskG487);
			...
			float4 tex2DNode7_g382 = tex2D( _Diffusemap3, ( uv_Diffusemap3 * TilingLayer3480 ) );
			...
			float clampResult15_g382 = clamp( _Desaturation3 , 0.0 , 1.0 );
			float3 desaturateInitialColor16_g382 = (temp_cast_17 + (pow( tex2DNode7_g382 , temp_cast_14 ) - temp_cast_15) * (temp_cast_18 - temp_cast_17) / (temp_cast_16 - temp_cast_15)).rgb;
			float desaturateDot16_g382 = dot( desaturateInitialColor16_g382, float3( 0.299, 0.587, 0.114 ));
			float3 desaturateVar16_g382 = lerp( desaturateInitialColor16_g382, desaturateDot16_g382.xxx, clampResult15_g382 );
			float4 lerpResult476 = lerp( lerpResult251 , ( _Color3 * float4( desaturateVar16_g382 , 0.0 ) ) , MaskG487);
			float4 temp_output_553_0 = ( _CiconiaStudioStandardMultiLayer * lerpResult476 );
			...
			float fresnelNdotV4_g427 = dot( ase_worldNormal, ase_worldViewDir );
			float fresnelNode4_g427 = ( -0.05 + 1.0 * pow( 1.0 - fresnelNdotV4_g427, 6.0 ) );
			float4 Emissivemap375 = ( ( _EmissionColor * tex2D( _EmissionMap, ( uv_EmissionMap * TilingLayer1478 ) ) * _EmissiveIntensity1 ) + ( ( ( 0.95 * fresnelNode4_g427 ) + 0.05 ) * _FresnelStrength ) );
			o.Emission = Emissivemap375.rgb;
			float2 uv_MetallicGlossMap = i.uv_texcoord * _MetallicGlossMap_ST.xy + _MetallicGlossMap_ST.zw;
			float4 tex2DNode3_g418 = tex2D( _MetallicGlossMap, ( uv_MetallicGlossMap * TilingLayer1478 ) );
			float4 lerpResult266 = lerp( ( _SpecularColor1 * tex2DNode3_g418 * _Metallic ) , ( _SpecularColor2 * _Metallic2 ) , MaskR246);
			float4 lerpResult499 = lerp( lerpResult266 , ( _SpecularColor3 * _Metallic3 ) , MaskG487);
			float fresnelNdotV4_g426 = dot( ase_worldNormal, ase_worldViewDir );
			float fresnelNode4_g426 = ( -0.05 + 1.0 * pow( 1.0 - fresnelNdotV4_g426, 6.0 ) );
			float4 Metallic274 = ( lerpResult499 + ( ( ( 0.95 * fresnelNode4_g426 ) + 0.05 ) * _FresnelStrength ) );
			o.Metallic = Metallic274.r;
			float lerpResult336 = lerp( ( tex2DNode3_g418.a * _Glossiness ) , _Roughness2 , MaskR246);
			...
			float blendOpSrc2_g429 = tex2D( _OcclusionMap, ( uv_OcclusionMap * TilingLayer1478 ) ).r;
			float blendOpDest2_g429 = ( 1.0 - _AoIntensity1 );
			float Aomap351 = ( saturate( ( 1.0 - ( 1.0 - blendOpSrc2_g429 ) * ( 1.0 - blendOpDest2_g429 ) ) ));
