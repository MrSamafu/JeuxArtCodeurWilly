Shader "Ciconia Studio/Shader Painter/SpecGloss/2Sided/Wind VertexOffset (DirectX 11 Only)"
{
	Properties
	{
	...
	SubShader
	{
		...
		void surf( Input i , inout SurfaceOutputStandardSpecular o )
		{
			...
			float3 lerpResult5_g363 = lerp( float3(0,0,1) , UnpackNormal( tex2D( _BumpMap, ( uv_BumpMap * _TilingLayer1 ) ) ) , _NormalIntensity1);
			float3 Normalmap338 = lerpResult5_g363;
			...
			float4 temp_output_504_0 = ( _CiconiaStudiowindvertexoffset * lerp(( _Color * float4( desaturateVar16_g349 , 0.0 ) ),float4( lerpResult27_g349 , 0.0 ),_VisualizeWind) * _2Sided );
			float3 linearToGamma138 = LinearToGammaSpace( temp_output_504_0.rgb );
			float clampResult389 = clamp( _Ambientlight , 1.0 , 9.0 );
			float4 lerpResult139 = lerp( ( float4( linearToGamma138 , 0.0 ) * UNITY_LIGHTMODEL_AMBIENT ) , temp_output_504_0 , clampResult389);
			...
			float4 tex2DNode3_g362 = tex2D( _SpecGlossMap, ( uv_SpecGlossMap * _TilingLayer1 ) );
			float4 temp_output_441_0 = ( _SpecularColor1 * tex2DNode3_g362 * _SpecularIntensity1 );
			float4 Specularmap274 = temp_output_441_0;
			o.Specular = Specularmap274.rgb;
			float Glossiness275 = ( tex2DNode3_g362.a * _Glossiness1 );
			o.Smoothness = Glossiness275;
			float2 uv_OcclusionMap = i.uv_texcoord * _OcclusionMap_ST.xy + _OcclusionMap_ST.zw;
			float blendOpSrc2_g364 = tex2D( _OcclusionMap, ( uv_OcclusionMap * _TilingLayer1 ) ).r;
			float blendOpDest2_g364 = ( 1.0 - _AoIntensity1 );
			float Aomap351 = ( saturate( ( 1.0 - ( 1.0 - blendOpSrc2_g364 ) * ( 1.0 - blendOpDest2_g364 ) ) ));
