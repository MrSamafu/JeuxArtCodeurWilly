Shader "Ciconia Studio/Shader Painter/SpecGloss/2Sided/Water VertexOffset (DirectX 11 Only)"
{
	Properties
	{
	...
	SubShader
	{
		...
		void vertexDataFunc( inout appdata_full v )
		{
			...
			float3 clampResult17_g443 = clamp( _ZAxis , float3( -100,-100,-100 ) , float3( 0,0,0 ) );
			v.vertex.xyz += ( ( MaskTex582 * ase_vertexNormal ) * clampResult17_g443 );
		}

		void surf( Input i , inout SurfaceOutputStandardSpecular o )
		{
			float2 uv_BumpMap = i.uv_texcoord * _BumpMap_ST.xy + _BumpMap_ST.zw;
			float3 lerpResult5_g419 = lerp( float3(0,0,1) , UnpackNormal( tex2D( _BumpMap, ( uv_BumpMap * _TilingLayer1 ) ) ) , _NormalIntensity1);
			float3 temp_output_443_0 = lerpResult5_g419;
			float3 _VectorBleu = float3(0,0,1);
			float2 uv_Normalmap2 = i.uv_texcoord * _Normalmap2_ST.xy + _Normalmap2_ST.zw;
			float cos12_g418 = cos( _Angle1 );
			float sin12_g418 = sin( _Angle1 );
			float2 rotator12_g418 = mul( ( uv_Normalmap2 * _TilingLayer2 ) - float2( 0.5,0.5 ) , float2x2( cos12_g418 , -sin12_g418 , sin12_g418 , cos12_g418 )) + float2( 0.5,0.5 );
			float2 temp_output_18_0_g418 = ( i.uv_texcoord / 4.0 );
			float2 panner16_g418 = ( ( _Time.y * _AnimationSpeed1 ) * float2( 0,0.6 ) + temp_output_18_0_g418);
			float3 tex2DNode4_g418 = UnpackNormal( tex2D( _Normalmap2, ( rotator12_g418 + panner16_g418 ) ) );
			float3 lerpResult5_g418 = lerp( _VectorBleu , tex2DNode4_g418 , _NormalIntensity2);
			float cos35_g418 = cos( _Angle2 );
			float sin35_g418 = sin( _Angle2 );
			float2 rotator35_g418 = mul( ( uv_Normalmap2 * _TilingLayer2 ) - float2( 0.5,0.5 ) , float2x2( cos35_g418 , -sin35_g418 , sin35_g418 , cos35_g418 )) + float2( 0.5,0.5 );
			float2 panner33_g418 = ( ( _Time.y * _AnimationSpeed2 ) * float2( 0,0.6 ) + temp_output_18_0_g418);
			float3 tex2DNode21_g418 = UnpackNormal( tex2D( _Normalmap2, ( rotator35_g418 + ( 1.0 - panner33_g418 ) ) ) );
			float3 lerpResult40_g418 = lerp( _VectorBleu , tex2DNode21_g418 , _NormalIntensity2);
			float3 temp_output_556_0 = BlendNormals( lerpResult5_g418 , lerpResult40_g418 );
			float2 uv_MainTex = i.uv_texcoord * _MainTex_ST.xy + _MainTex_ST.zw;
			float4 tex2DNode7_g391 = tex2D( _MainTex, ( uv_MainTex * _TilingLayer1 ) );
			float ADiffuse1210 = tex2DNode7_g391.a;
			float clampResult11_g417 = clamp( ( _Spread + (( 1.0 + _Contrast ) + (ADiffuse1210 - 0.0) * (-_Contrast - ( 1.0 + _Contrast )) / (1.0 - 0.0)) ) , 0.0 , 1.0 );
			...
			float temp_output_21_0_g417 = MaskBrush232;
			float blendOpSrc18_g417 = clampResult11_g417;
			float blendOpDest18_g417 = saturate( ( (-1.0 + (3.0 - 0.0) * (1.0 - -1.0) / (1.0 - 0.0)) + (( 1.0 + -6.0 ) + (temp_output_21_0_g417 - 0.0) * (6.0 - ( 1.0 + -6.0 )) / (1.0 - 0.0)) ) );
			float blendOpSrc31_g417 = lerp(clampResult11_g417,( saturate( ( blendOpSrc18_g417 * blendOpDest18_g417 ) )),_Heightmapcontstraintbymask);
			float blendOpDest31_g417 = temp_output_21_0_g417;
			float Mask246 = ( ( saturate( ( blendOpSrc31_g417 + blendOpDest31_g417 ) )) * _MaskOpacity );
			...
			float4 temp_output_606_0 = ( _CiconiaStudiowatervertexoffset * lerpResult251 * _2Sided );
			float3 linearToGamma138 = LinearToGammaSpace( temp_output_606_0.rgb );
			float clampResult389 = clamp( _Ambientlight , 1.0 , 9.0 );
			float4 lerpResult139 = lerp( ( float4( linearToGamma138 , 0.0 ) * UNITY_LIGHTMODEL_AMBIENT ) , temp_output_606_0 , clampResult389);
			...
			float fresnelNdotV4_g420 = dot( ase_worldNormal, ase_worldViewDir );
			float fresnelNode4_g420 = ( -0.05 + 1.0 * pow( 1.0 - fresnelNdotV4_g420, 6.0 ) );
			float4 texCUBENode507 = texCUBElod( _Cubemap, float4( WorldReflectionVector( i , Normalmap338 ), _BlurReflection) );
			float4 Emissivemap375 = ( ( _EmissionColor * tex2D( _EmissionMap, ( uv_EmissionMap * _TilingLayer1 ) ) * _EmissiveIntensity1 ) + ( ( ( ( 0.95 * fresnelNode4_g420 ) + 0.05 ) * _FresnelStrength ) + ( texCUBENode507 * ( texCUBENode507.a * ( _ReflectionIntensity / 10.0 ) * Mask246 ) ) ) );
			o.Emission = Emissivemap375.rgb;
			float2 uv_SpecGlossMap = i.uv_texcoord * _SpecGlossMap_ST.xy + _SpecGlossMap_ST.zw;
			float4 tex2DNode3_g439 = tex2D( _SpecGlossMap, ( uv_SpecGlossMap * _TilingLayer1 ) );
			float temp_output_17_0_g439 = Mask246;
			float4 lerpResult16_g439 = lerp( ( _SpecularColor1 * tex2DNode3_g439 * _SpecularIntensity1 ) , ( _SpecularColor2 * tex2DNode3_g439 * _SpecularIntensity2 ) , temp_output_17_0_g439);
			...
			float4 Specularmap274 = ( lerpResult16_g439 + ( ( ( 0.95 * fresnelNode4_g441 ) + 0.05 ) * _FresnelStrength ) );
			o.Specular = Specularmap274.rgb;
			float lerpResult22_g439 = lerp( ( tex2DNode3_g439.a * _Glossiness1 ) , _Glossiness2 , temp_output_17_0_g439);
			float Glossiness275 = lerpResult22_g439;
			o.Smoothness = Glossiness275;
			float2 uv_OcclusionMap = i.uv_texcoord * _OcclusionMap_ST.xy + _OcclusionMap_ST.zw;
			float blendOpSrc2_g442 = tex2D( _OcclusionMap, ( uv_OcclusionMap * _TilingLayer1 ) ).r;
			float blendOpDest2_g442 = ( 1.0 - _AoIntensity1 );
			float blendOpSrc42_g418 = tex2DNode4_g418.b;
			float blendOpDest42_g418 = tex2DNode21_g418.b;
			float SpecularWave549 = ( saturate( (( blendOpDest42_g418 > 0.5 ) ? ( 1.0 - ( 1.0 - 2.0 * ( blendOpDest42_g418 - 0.5 ) ) * ( 1.0 - blendOpSrc42_g418 ) ) : ( 2.0 * blendOpDest42_g418 * blendOpSrc42_g418 ) ) ));
			float blendOpSrc552 = SpecularWave549;
			float blendOpDest552 = 0.0;
			float lerpResult362 = lerp( ( saturate( ( 1.0 - ( 1.0 - blendOpSrc2_g442 ) * ( 1.0 - blendOpDest2_g442 ) ) )) , ( saturate( ( 1.0 - ( ( 1.0 - blendOpDest552) / blendOpSrc552) ) )) , Mask246);
