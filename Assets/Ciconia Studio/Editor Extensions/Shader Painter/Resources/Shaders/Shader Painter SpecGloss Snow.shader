Shader "Ciconia Studio/Shader Painter/SpecGloss/Snow"
{
	Properties
	{
	...
	SubShader
	{
		...
		void surf( Input i , inout SurfaceOutputStandardSpecular o )
		{
			...
			float3 lerpResult5_g218 = lerp( float3(0,0,1) , UnpackNormal( tex2D( _BumpMap, ( uv_BumpMap * _TilingLayer1 ) ) ) , _NormalIntensity1);
			float3 temp_output_486_0 = lerpResult5_g218;
			float2 uv_Normalmap2 = i.uv_texcoord * _Normalmap2_ST.xy + _Normalmap2_ST.zw;
			float3 lerpResult5_g219 = lerp( float3(0,0,1) , UnpackNormal( tex2D( _Normalmap2, ( uv_Normalmap2 * _TilingLayer2 ) ) ) , _NormalIntensity2);
			float3 temp_output_596_0 = lerpResult5_g219;
			float2 uv_MainTex = i.uv_texcoord * _MainTex_ST.xy + _MainTex_ST.zw;
			float4 tex2DNode7_g214 = tex2D( _MainTex, ( uv_MainTex * _TilingLayer1 ) );
			float ADiffuse1210 = tex2DNode7_g214.a;
			float clampResult11_g215 = clamp( ( _Spread + (( 1.0 + _Contrast ) + (ADiffuse1210 - 0.0) * (-_Contrast - ( 1.0 + _Contrast )) / (1.0 - 0.0)) ) , 0.0 , 1.0 );
			float2 uv_Brush = i.uv_texcoord * _Brush_ST.xy + _Brush_ST.zw;
			float2 appendResult19_g213 = (float2(_Brushposition.x , _Brushposition.y));
			float cos23_g213 = cos( _Rotate );
			float sin23_g213 = sin( _Rotate );
			float2 rotator23_g213 = mul( ( uv_Brush + ( appendResult19_g213 * _Pensize ) ) - float2( 0.5,0.5 ) , float2x2( cos23_g213 , -sin23_g213 , sin23_g213 , cos23_g213 )) + float2( 0.5,0.5 );
			float temp_output_25_0_g213 = ( tex2D( _Brush, rotator23_g213 ).a * _Opacity );
			float2 uv_Mask = i.uv_texcoord * _Mask_ST.xy + _Mask_ST.zw;
			float temp_output_33_0_g213 = ( tex2D( _Mask, uv_Mask ).r * _OpacityDepth );
			float temp_output_31_0_g213 = min( ( 1.0 - temp_output_25_0_g213 ) , temp_output_33_0_g213 );
			float ifLocalVar38_g213 = 0;
			if( lerp(0.0,1.0,_Erase) == 0.0 )
				ifLocalVar38_g213 = max( temp_output_25_0_g213 , temp_output_33_0_g213 );
			else
				ifLocalVar38_g213 = temp_output_31_0_g213;
			float MaskBrush232 = ifLocalVar38_g213;
			float temp_output_21_0_g215 = MaskBrush232;
			float blendOpSrc18_g215 = clampResult11_g215;
			float blendOpDest18_g215 = saturate( ( (-1.0 + (3.0 - 0.0) * (1.0 - -1.0) / (1.0 - 0.0)) + (( 1.0 + -6.0 ) + (temp_output_21_0_g215 - 0.0) * (6.0 - ( 1.0 + -6.0 )) / (1.0 - 0.0)) ) );
			float blendOpSrc31_g215 = lerp(clampResult11_g215,( saturate( ( blendOpSrc18_g215 * blendOpDest18_g215 ) )),_Heightmapcontstraintbymask);
			float blendOpDest31_g215 = temp_output_21_0_g215;
			float Mask246 = ( ( saturate( ( blendOpSrc31_g215 + blendOpDest31_g215 ) )) * _MaskOpacity );
			...
			float clampResult15_g214 = clamp( _Desaturation1 , 0.0 , 1.0 );
			float3 desaturateInitialColor16_g214 = (temp_cast_3 + (pow( tex2DNode7_g214 , temp_cast_0 ) - temp_cast_1) * (temp_cast_4 - temp_cast_3) / (temp_cast_2 - temp_cast_1)).rgb;
			float desaturateDot16_g214 = dot( desaturateInitialColor16_g214, float3( 0.299, 0.587, 0.114 ));
			float3 desaturateVar16_g214 = lerp( desaturateInitialColor16_g214, desaturateDot16_g214.xxx, clampResult15_g214 );
			float2 uv_Diffusemap2 = i.uv_texcoord * _Diffusemap2_ST.xy + _Diffusemap2_ST.zw;
			float4 tex2DNode7_g212 = tex2D( _Diffusemap2, ( uv_Diffusemap2 * _TilingLayer2 ) );
			...
			float clampResult15_g212 = clamp( _Desaturation2 , 0.0 , 1.0 );
			float3 desaturateInitialColor16_g212 = (temp_cast_10 + (pow( tex2DNode7_g212 , temp_cast_7 ) - temp_cast_8) * (temp_cast_11 - temp_cast_10) / (temp_cast_9 - temp_cast_8)).rgb;
			float desaturateDot16_g212 = dot( desaturateInitialColor16_g212, float3( 0.299, 0.587, 0.114 ));
			float3 desaturateVar16_g212 = lerp( desaturateInitialColor16_g212, desaturateDot16_g212.xxx, clampResult15_g212 );
			float4 lerpResult251 = lerp( ( _Color * float4( desaturateVar16_g214 , 0.0 ) ) , ( _Color2 * float4( desaturateVar16_g212 , 0.0 ) ) , Mask246);
			float4 temp_output_482_0 = ( _CiconiaStudioSnow * lerpResult251 );
			...
			float3 worldToObj45_g216 = mul( unity_WorldToObject, float4( ase_worldPos, 1 ) ).xyz;
			float3 ase_worldViewDir = normalize( UnityWorldSpaceViewDir( ase_worldPos ) );
			float3 temp_output_9_0_g216 = ( ( ( worldToObj45_g216 / 4.0 ) - ase_worldViewDir ) * ( _TilingCrystals / 10.0 ) );
			float3 weightedBlendVar32_g216 = abs( ase_vertexNormal );
			float4 weightedBlend32_g216 = ( weightedBlendVar32_g216.x*tex2D( _Crystalsmap, (temp_output_9_0_g216).yz ) + weightedBlendVar32_g216.y*tex2D( _Crystalsmap, (temp_output_9_0_g216).xz ) + weightedBlendVar32_g216.z*tex2D( _Crystalsmap, (temp_output_9_0_g216).xy ) );
			float clampResult33_g216 = clamp( _Desaturatecrystals , 0.0 , 1.0 );
			float3 desaturateInitialColor36_g216 = weightedBlend32_g216.rgb;
			float desaturateDot36_g216 = dot( desaturateInitialColor36_g216, float3( 0.299, 0.587, 0.114 ));
			float3 desaturateVar36_g216 = lerp( desaturateInitialColor36_g216, desaturateDot36_g216.xxx, clampResult33_g216 );
			float lerpResult30_g216 = lerp( -3.5 , 1.0 , _MaskAmount);
			...
			float ADiffuse2604 = tex2DNode7_g212.a;
			...
			float4 SnowEmission505 = ( ( float4( ( desaturateVar36_g216 * ( _CrystalsIntensity * 20.0 ) ) , 0.0 ) * saturate( ( lerpResult30_g216 + (temp_cast_22 + (tex2D( _DotMask, ( uv_DotMask * _TilingMask ) ) - temp_cast_20) * (temp_cast_23 - temp_cast_22) / (temp_cast_21 - temp_cast_20)) ) ) ) * AoLayer2611 );
			...
			float fresnelNdotV4_g226 = dot( ase_worldNormal, ase_worldViewDir );
			float fresnelNode4_g226 = ( -0.05 + 1.0 * pow( 1.0 - fresnelNdotV4_g226, 6.0 ) );
			float4 Emissivemap375 = ( lerpResult378 + ( ( ( 0.95 * fresnelNode4_g226 ) + 0.05 ) * _FresnelStrength ) );
			o.Emission = Emissivemap375.rgb;
			float2 uv_SpecGlossMap = i.uv_texcoord * _SpecGlossMap_ST.xy + _SpecGlossMap_ST.zw;
			float4 tex2DNode3_g222 = tex2D( _SpecGlossMap, ( uv_SpecGlossMap * _TilingLayer1 ) );
			float2 uv_SpecularmapGlossA2 = i.uv_texcoord * _SpecularmapGlossA2_ST.xy + _SpecularmapGlossA2_ST.zw;
			float4 tex2DNode3_g221 = tex2D( _SpecularmapGlossA2, ( uv_SpecularmapGlossA2 * _TilingLayer2 ) );
			float4 lerpResult266 = lerp( ( _SpecularColor1 * tex2DNode3_g222 * _SpecularIntensity1 ) , ( _SpecularColor2 * tex2DNode3_g221 * _SpecularIntensity2 ) , Mask246);
			float fresnelNdotV4_g224 = dot( ase_worldNormal, ase_worldViewDir );
			float fresnelNode4_g224 = ( -0.05 + 1.0 * pow( 1.0 - fresnelNdotV4_g224, 6.0 ) );
			float4 Specularmap274 = ( lerpResult266 + ( ( ( 0.95 * fresnelNode4_g224 ) + 0.05 ) * _FresnelStrength ) );
			o.Specular = Specularmap274.rgb;
			float lerpResult336 = lerp( ( tex2DNode3_g222.a * _Glossiness1 ) , ( tex2DNode3_g221.a * _Glossiness2 ) , Mask246);
			float Glossiness275 = lerpResult336;
			o.Smoothness = Glossiness275;
			float2 uv_OcclusionMap = i.uv_texcoord * _OcclusionMap_ST.xy + _OcclusionMap_ST.zw;
			float blendOpSrc2_g225 = tex2D( _OcclusionMap, ( uv_OcclusionMap * _TilingLayer1 ) ).r;
			float blendOpDest2_g225 = ( 1.0 - _AoIntensity1 );
			float lerpResult362 = lerp( ( saturate( ( 1.0 - ( 1.0 - blendOpSrc2_g225 ) * ( 1.0 - blendOpDest2_g225 ) ) )) , temp_output_607_0 , Mask246);
