Shader "Ciconia Studio/Shader Painter/SpecGloss/2Sided/Transparent/CutOut"
{
	Properties
	{
	...
	SubShader
	{
		...
		void surf( Input i , inout SurfaceOutputStandardSpecular o )
		{
			...
			float3 lerpResult5_g316 = lerp( float3(0,0,1) , UnpackNormal( tex2D( _BumpMap, ( uv_BumpMap * _TilingLayer1 ) ) ) , _NormalIntensity1);
			float3 Normalmap338 = lerpResult5_g316;
			o.Normal = Normalmap338;
			float2 uv_MainTex = i.uv_texcoord * _MainTex_ST.xy + _MainTex_ST.zw;
			float4 tex2DNode7_g234 = tex2D( _MainTex, ( uv_MainTex * _TilingLayer1 ) );
			...
			float4 temp_output_473_0 = ( _CiconiaStudiocutout * ( _Color * float4( desaturateVar16_g234 , 0.0 ) ) * _2Sided );
			float3 linearToGamma138 = LinearToGammaSpace( temp_output_473_0.rgb );
			float clampResult389 = clamp( _Ambientlight , 1.0 , 9.0 );
			float4 lerpResult139 = lerp( ( float4( linearToGamma138 , 0.0 ) * UNITY_LIGHTMODEL_AMBIENT ) , temp_output_473_0 , clampResult389);
			...
			float fresnelNdotV4_g312 = dot( ase_worldNormal, ase_worldViewDir );
			float fresnelNode4_g312 = ( -0.05 + 1.0 * pow( 1.0 - fresnelNdotV4_g312, 6.0 ) );
			float4 Emissivemap375 = ( ( _EmissionColor * tex2D( _EmissionMap, ( uv_EmissionMap * _TilingLayer1 ) ) * _EmissiveIntensity1 ) + ( ( ( 0.95 * fresnelNode4_g312 ) + 0.05 ) * _FresnelStrength ) );
			o.Emission = Emissivemap375.rgb;
			float2 uv_SpecGlossMap = i.uv_texcoord * _SpecGlossMap_ST.xy + _SpecGlossMap_ST.zw;
			float4 tex2DNode3_g314 = tex2D( _SpecGlossMap, ( uv_SpecGlossMap * _TilingLayer1 ) );
			float fresnelNdotV4_g311 = dot( ase_worldNormal, ase_worldViewDir );
			float fresnelNode4_g311 = ( -0.05 + 1.0 * pow( 1.0 - fresnelNdotV4_g311, 6.0 ) );
			float4 Specularmap274 = ( ( _SpecularColor1 * tex2DNode3_g314 * _SpecularIntensity1 ) + ( ( ( 0.95 * fresnelNode4_g311 ) + 0.05 ) * _FresnelStrength ) );
			o.Specular = Specularmap274.rgb;
			float Glossiness275 = ( tex2DNode3_g314.a * _Glossiness1 );
			o.Smoothness = Glossiness275;
			float2 uv_OcclusionMap = i.uv_texcoord * _OcclusionMap_ST.xy + _OcclusionMap_ST.zw;
			float blendOpSrc2_g315 = tex2D( _OcclusionMap, ( uv_OcclusionMap * _TilingLayer1 ) ).r;
			float blendOpDest2_g315 = ( 1.0 - _AoIntensity1 );
			float Aomap351 = ( saturate( ( 1.0 - ( 1.0 - blendOpSrc2_g315 ) * ( 1.0 - blendOpDest2_g315 ) ) ));
			o.Occlusion = Aomap351;
			o.Alpha = 1;
			float ADiffuse1210 = tex2DNode7_g234.a;
			float2 uv_Brush = i.uv_texcoord * _Brush_ST.xy + _Brush_ST.zw;
			float2 appendResult19_g298 = (float2(_Brushposition.x , _Brushposition.y));
			float cos23_g298 = cos( _Rotate );
			float sin23_g298 = sin( _Rotate );
			float2 rotator23_g298 = mul( ( uv_Brush + ( appendResult19_g298 * _Pensize ) ) - float2( 0.5,0.5 ) , float2x2( cos23_g298 , -sin23_g298 , sin23_g298 , cos23_g298 )) + float2( 0.5,0.5 );
			float temp_output_25_0_g298 = ( tex2D( _Brush, rotator23_g298 ).a * _Opacity );
			float2 uv_Mask = i.uv_texcoord * _Mask_ST.xy + _Mask_ST.zw;
			float temp_output_33_0_g298 = ( tex2D( _Mask, uv_Mask ).r * _OpacityDepth );
			float temp_output_31_0_g298 = min( ( 1.0 - temp_output_25_0_g298 ) , temp_output_33_0_g298 );
			float ifLocalVar38_g298 = 0;
			if( lerp(0.0,1.0,_Erase) == 0.0 )
				ifLocalVar38_g298 = max( temp_output_25_0_g298 , temp_output_33_0_g298 );
			else
				ifLocalVar38_g298 = temp_output_31_0_g298;
			float MaskBrush232 = ifLocalVar38_g298;
