Shader "Ciconia Studio/Shader Painter/SpecGloss/Standard Multi Layers"
{
	...
	SubShader
	{
		...
		void surf( Input i , inout SurfaceOutputStandardSpecular o )
		{
			...
			float3 lerpResult5_g392 = lerp( float3(0,0,1) , UnpackNormal( tex2D( _BumpMap, ( uv_BumpMap * TilingLayer1478 ) ) ) , _NormalIntensity1);
			float3 temp_output_443_0 = lerpResult5_g392;
			float2 uv_Normalmap2 = i.uv_texcoord * _Normalmap2_ST.xy + _Normalmap2_ST.zw;
			float TilingLayer2479 = _TilingLayer2;
			float3 lerpResult5_g393 = lerp( float3(0,0,1) , UnpackNormal( tex2D( _Normalmap2, ( uv_Normalmap2 * TilingLayer2479 ) ) ) , _NormalIntensity2);
			...
			float3 lerpResult347 = lerp( temp_output_443_0 , lerpResult5_g393 , MaskR246);
			float2 uv_Normalmap3 = i.uv_texcoord * _Normalmap3_ST.xy + _Normalmap3_ST.zw;
			float TilingLayer3480 = _TilingLayer3;
			float3 lerpResult5_g400 = lerp( float3(0,0,1) , UnpackNormal( tex2D( _Normalmap3, ( uv_Normalmap3 * TilingLayer3480 ) ) ) , _NormalIntensity3);
			...
			float3 lerpResult524 = lerp( lerpResult347 , lerpResult5_g400 , MaskG487);
			...
			float4 temp_output_548_0 = ( _CiconiaStudioStandardMultiLayer * lerpResult476 );
			...
			float fresnelNdotV4_g408 = dot( ase_worldNormal, ase_worldViewDir );
			float fresnelNode4_g408 = ( -0.05 + 1.0 * pow( 1.0 - fresnelNdotV4_g408, 6.0 ) );
			float4 Emissivemap375 = ( ( _EmissionColor * tex2D( _EmissionMap, ( uv_EmissionMap * TilingLayer1478 ) ) * _EmissiveIntensity1 ) + ( ( ( 0.95 * fresnelNode4_g408 ) + 0.05 ) * _FresnelStrength ) );
			o.Emission = Emissivemap375.rgb;
			float2 uv_SpecGlossMap = i.uv_texcoord * _SpecGlossMap_ST.xy + _SpecGlossMap_ST.zw;
			float4 tex2DNode3_g399 = tex2D( _SpecGlossMap, ( uv_SpecGlossMap * TilingLayer1478 ) );
			float4 lerpResult266 = lerp( ( _SpecularColor1 * tex2DNode3_g399 * _SpecularIntensity1 ) , ( _SpecularColor2 * _SpecularIntensity2 ) , MaskR246);
			float4 lerpResult499 = lerp( lerpResult266 , ( _SpecularColor3 * _SpecularIntensity3 ) , MaskG487);
			float fresnelNdotV4_g407 = dot( ase_worldNormal, ase_worldViewDir );
			float fresnelNode4_g407 = ( -0.05 + 1.0 * pow( 1.0 - fresnelNdotV4_g407, 6.0 ) );
			float4 Specularmap274 = ( lerpResult499 + ( ( ( 0.95 * fresnelNode4_g407 ) + 0.05 ) * _FresnelStrength ) );
			o.Specular = Specularmap274.rgb;
			float lerpResult336 = lerp( ( tex2DNode3_g399.a * _Glossiness1 ) , _Glossiness2 , MaskR246);
			...
			float blendOpSrc2_g410 = tex2D( _OcclusionMap, ( uv_OcclusionMap * TilingLayer1478 ) ).r;
			float blendOpDest2_g410 = ( 1.0 - _AoIntensity1 );
			float Aomap351 = ( saturate( ( 1.0 - ( 1.0 - blendOpSrc2_g410 ) * ( 1.0 - blendOpDest2_g410 ) ) ));
